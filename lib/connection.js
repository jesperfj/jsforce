 ...
 * @param {String} [options.forceUrl] - URL in the form force://clientId:clientSecret:refreshToken@instanceUrl. Will override other options.
...
var Connection = module.exports = function(options) {
  ...
  if(options.forceUrl) {
    var p = options.forceUrl.match(/^force:\/\/([^:]*):([^:]*):([^@]*)@([^\/]*)\/$/)
    if(p && p.length==5) {
      options.clientId = p[1]
      options.clientSecret = p[2]
      options.refreshToken = p[3]
      options.instanceUrl = 'https://'+p[4]
      options.loginUrl = options.instanceUrl
    } else {
      throw new Error("Malformed forceUrl: "+options.forceUrl+". Must be in form force://clientId:clientSecret:refreshToken@instanceUrl")
    }
  }
